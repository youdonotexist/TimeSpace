...
public class FollowTrackingCamera : MonoBehaviour
{
	...
	void Start(){
		...
		emptyTarget = transform.position;
	...
	void Update() {
		if (!target) {
			...
			emptyTarget += new Vector3(direction.x * Time.deltaTime * 3.0f, direction.y * Time.deltaTime * 3.0f, 0.0f);
		}
	...
	void LateUpdate(){
		...
		if (target != null) {
			...
			transform.position = emptyTarget + zoomResult;
		}
